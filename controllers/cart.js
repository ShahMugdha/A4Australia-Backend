...
export const getCartList = async (req, res) => {
  try {
    const cart = await Cart.find({ user: req.userData })
      .populate("user")
      .populate("cart.product");
    if (!cart) {
      return res
        .status(200)
        .json({ success: false, message: "cart list not found" });
    }
    return res
      .status(200)
      .json({ success: true, message: "retrieved cart list", result: cart });
  } catch (err) {
    return res
      .status(500)
...
export const getParticularProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      return res
        .status(200)
        .json({ success: false, message: "product by this title not found" });
    }
    return res
      .status(200)
      .json({
        success: true,
        message: "retrieved product by title",
        result: product,
      });
  } catch (err) {
    return res
  ...
  try {
    ...
    if (!userExists) {
      ...
      if (!cart) {
        return res
          .status(200)
          .json({ success: false, message: "new cart not created" });
      }
      return res
        .status(201)
        .json({
          success: true,
          message: "new cart created with one product",
          result: cart,
        });
    }
    ...
    if (!productExists) {
      const newProduct = await Cart.findOneAndUpdate(
        { user: req.userData },
        {
          $addToSet: { cart: { product, size, price: product.price } },
      ...
      if (!newProduct) {
        return res
          .status(200)
          .json({ success: false, message: "new product not added to cart" });
      }
      return res
        .status(201)
        .json({
  ...
  try {
    ...
    const existingSize = await Cart.findOne(
      {
        user: req.userData,
        cart: {
          $elemMatch: { product: { _id: productId }, size: updatedSize },
        },
      },
      { "cart.$": 1 }
    );
    ...
    if (existingSize) {
      const updatedProductQuantity = await Cart.findOneAndUpdate(
        ...
        {
          $inc: {
            "cart.$.price": existingSize.cart[0].price,
            "cart.$.quantity": existingSize.cart[0].quantity,
          },
        },
        { new: true }
      );
      ...
      if (updatedProductQuantity) {
        const deletedProductwithSameSize = await Cart.findOneAndUpdate(
          ...
          {
            $pull: {
              cart: { product: { _id: productId }, size: originalSize },
            },
          },
          { new: true }
        );
        ...
        return res
          ...
          .json({
            ...
            result: deletedProductwithSameSize,
          });
      }
      return res
        .status(200)
    ...
    const updatedProductSize = await Cart.findOneAndUpdate(
      {
        user: req.userData,
        cart: {
          $elemMatch: { product: { _id: productId }, size: originalSize },
        },
      },
    ...
    if (!updatedProductSize) {
      return res
        .status(200)
        .json({ success: false, message: "product size not updated" });
    }
    return res
      .status(201)
      .json({
        success: true,
        message: "product details updated with size",
...
export const updateProductQuantity = async (req, res) => {
  try {
    ...
    if (!updatedProductQuantity) {
      return res
        .status(200)
        .json({ success: false, message: "product details not updated" });
    }
    return res
      .status(201)
      .json({
...
export const deleteProductFromCart = async (req, res) => {
  try {
    ...
    if (!deletedProduct) {
      return res
        .status(200)
        .json({ success: false, message: "product not deleted from cart" });
    }
    ...
    if (!updatedQuantityAndPrice) {
      return res
        ...
        .json({
          success: false,
          message: "total quantity and price not updated",
        });
    }
    return res
      .status(200)
      .json({
        success: true,
        message: "product deleted from cart and quantity price updated",
        result: updatedQuantityAndPrice,
      });
  } catch (err) {
    return res
      .status(500)
...
export const moveProductToWishList = async (req, res) => {
  try {
    ...
    if (!movedProduct) {
      return res
        .status(200)
        .json({ success: false, message: "product not moved to wishlist" });
    }
    return res
      .status(200)
      .json({
...
export const deleteCart = async (req, res) => {
  try {
    ...
    if (!deletedCart) {
      return res
        .status(200)
        .json({ success: false, message: "cart not deleted" });
    }
    return res
      .status(200)
      .json({ success: true, message: "cart deleted", result: deletedCart });
  } catch (err) {
    return res
      .status(500)
      .json({ success: false, message: "something went wrong", result: err });
