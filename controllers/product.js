...
export const getProductList = async(req, res) => {
  try {
    ...
    if(!productList) {
      return res.status(404).json({success: false, message: "product list not found"});
    }
...
export const getProductsByCategory = async(req, res) => {
  try {
    ...
    if(!prodByCat) {
      return res.status(404).json({success: false, message: "product by this category not found"});
    }
