...
export const getCustomerOrder = async (req, res) => {
  try {
    ...
    const order = await Order.findOne(
      ...
      { "orderHistory.$": 1 }
    ).populate("user").populate("orderHistory.order.product");
    if (!order) {
      return res.status(200).json({ success: false, message: "order not found" });
    }
    return res.status(200).json({ success: true, message: "retrieved order", result: order });
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(200).json({ success: false, message: "orders not found" });
    }
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (cart && !userOrderExists) {
      ...
      if (!newOrder) {
        return res.status(200).json({ success: false, message: "new order not added" });
      }
