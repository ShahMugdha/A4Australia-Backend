...
export const getMyOrder = async (req, res) => {
  try {
    const order = await Order.findOne({ user: req.userData })
      .populate("user")
      .populate("orderHistory.order.product");
    if (!order) {
      return res
        .status(200)
    ...
    return res
      .status(200)
      .json({ success: true, message: "retrieved order", result: order });
  } catch (err) {
    return res
      .status(500)
...
export const getCustomerOrder = async (req, res) => {
  try {
    ...
    const order = await Order.findOne(
      { orderHistory: { $elemMatch: { paymentIntentId } } },
      { "orderHistory.$": 1 }
    )
      .populate("user")
      .populate("orderHistory.order.product");
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "retrieved all orders", result: order });
  } catch (err) {
    return res
      .status(500)
      .json({ success: false, message: "something went wrong", result: err });
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (cart && !userOrderExists) {
      ...
      return res
        .status(201)
        .json({ success: true, message: "order created", result: orderCreate });
      ...
      if (!newOrder) {
        return res
          .status(200)
          .json({ success: false, message: "new order not added" });
      }
      return res
        .status(201)
        .json({ success: true, message: "new order added", result: newOrder });
    } else if (!cart)
      return res
        .status(409)
